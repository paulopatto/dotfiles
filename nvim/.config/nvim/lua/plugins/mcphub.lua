return {
  ...
  config = function()
    ...
    if not vim.loop.fs_stat(mcphub_config_path) then
      vim.ui.select(
        { "Sim", "Não" },
        { prompt = "Arquivo de configuração do MCPHub não encontrado. Deseja criar?" },
        function(choice)
          if choice == "Sim" then
            -- Criar diretório se não existir
            if not vim.loop.fs_stat(mcphub_config_dir) then
              vim.loop.fs_mkdir(mcphub_config_dir, 0700, function(err)
                if err then
                  vim.notify("Erro ao criar diretório: " .. err, vim.log.levels.ERROR)
                  return
                end
              end)
            end
            ...
            -- Criar arquivo
            local file = io.open(mcphub_config_path, "w")
            if file then
              file:write("{}") -- Escrever um JSON vazio
              file:close()
              vim.notify("Arquivo de configuração do MCPHub criado em " .. mcphub_config_path, vim.log.levels.INFO)
            else
              vim.notify("Erro ao criar arquivo de configuração do MCPHub.", vim.log.levels.ERROR)
            end
          end
        end
      )
